...
const CONFIG = {
  ...
  // SearchAPI.io (Google Search) Settings
  SEARCHAPI_URL: "https://www.searchapi.io/api/v1/search",
  SEARCHAPI_KEY: process.env.SEARCHAPI_KEY, // put this in your .env file
};
...
const tools = [
  {
    functionDeclarations: [
      ...
      // ---- Tool 5: Google Search ----
      // Performs a real, live Google search via SearchAPI.io
      {
        name: "googleSearch",
        description:
          "Search Google for current, real-world information (news, facts, people, current events, etc.) that the model may not know or that changes over time.",
        parameters: {
          type: "object",
          properties: {
            query: {
              type: "string",
              description:
                "The search query to look up on Google, e.g., 'west bengal current cm name'",
            },
          },
          required: ["query"],
        },
...
// --- Tool Implementation: googleSearch ---
// Calls the real Google Search API via SearchAPI.io (async, uses fetch)
async function googleSearch(query) {
  if (!CONFIG.SEARCHAPI_KEY) {
    return { query, error: "SEARCHAPI_KEY is not set in your .env file" };
  }
  ...
  try {
    ...
    const response = await fetch(`${CONFIG.SEARCHAPI_URL}?${params}`);

    if (!response.ok) {
      return { query, error: `SearchAPI request failed with status ${response.status}` };
    }

    const data = await response.json();

    // Pull out the most useful bits so the LLM isn't overwhelmed with raw payload
    ...
    return { query, error: `Search failed: ${error.message}` };
  }
}
